...
void ObjMallaIndexada::rellenarColores(Tupla3f colPrincipal){
    colPrin.clear();
    colSec.clear();
    for(int i=0;i<vertices.size();i++){
        colPrin.push_back(colPrincipal);
        colSec.push_back({0.0,0.0,0.0});
    }

}

Tupla3f ObjMallaIndexada::mostrarColores(){
  return colPrin[0];
}
